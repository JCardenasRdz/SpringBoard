...
df_transactions = pd.read_excel("./WineKMC.xlsx", sheetname=1,names=["customer_name", "offer_id"])
df_transactions['n'] = 1

# estimate total sales per offer_id
df_total_sales = pd.merge(df_offers[['offer_id']], df_transactions)
...
#est = sm.OLS(y, X).fit(); est.summary()
